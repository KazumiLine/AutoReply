# -*- coding: utf-8 -*-
from akad.ttypes import Message, ChatRoomAnnouncementContents,ContactSetting
...
class Talk(object):
    ...
    @loggedIn
    def templatepic(self,profile):
        a = self.profile.mid
        contentMetadata={
        'PREVIEW_URL': 'https://os.line.naver.jp/os/p/{}'.format(profile.mid),
        'AGENT_ICON': 'https://profile.line-scdn.net/{}'.format(profile.picturePath),
        'AGENT_NAME': profile.displayName,
        'AGENT_LINK': 'line://app/1616544281-elVzA8ng?type=foimage&img=https://os.line.naver.jp/os/p/{}'.format(a),
        'DOWNLOAD_URL': 'https://os.line.naver.jp/os/p/{}'.format(a)}
        return contentMetadata
    ...
    def deleteContact(self, mid):
        return self.talk.updateContactSetting(16,mid,ContactSetting.CONTACT_SETTING_DELETE,'True')
    ...
    @loggedIn
    def createChatRoomAnnouncement(self, to, text, msgid):
        announce = ChatRoomAnnouncementContents()
        announce.displayFields = 5
        announce.text = text
        announce.link = "line://nv/chatMsg?chatId={}&messageId={}".format(to,msgid)
        return self.talk.createChatRoomAnnouncement(0, to, 0, announce)
